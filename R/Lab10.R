...
for(val in 1:length(VSAParams$Pusle)) {
  if(VSAParams$slp[val] >= 0.01 & VSAParams$slp[val] <= 0.02)
    VSAParams$Pusle[val] = 0.6
  if(VSAParams$slp[val] >= 0.03 & VSAParams$slp[val] <= 0.05)
    VSAParams$Pusle[val] = 0.5
  if(VSAParams$slp[val] >= 0.06 & VSAParams$slp[val] <= 0.08)
    VSAParams$Pusle[val] = 0.5
  if(VSAParams$slp[val] >= 0.09 & VSAParams$slp[val] <= 0.12)
    VSAParams$Pusle[val] = 0.6
  print(VSAParams$slp[val])
...
TIC02 = CNmodel(CNmodeldf = TIC01, CNavg=VSAParams$CN[2], 
                declat=myflowgage$declat,declon=myflowgage$declon)
TIC02$qpeak=TIC02$Qpred/3600/24/1000*myflowgage$area/nTIclass*10^6 #m^3/sec
TIC02$sed=(TIC02$Qpred*TIC01$qpeak*myflowgage$area/nTIclass*100)^.56*MUSLE$KCPLSCFRG118[2]    # Eq. 4:1.1.1 SWAT Theory

TIC03$P=TIC02$Qpred+TIC03$P

TIC03 = CNmodel(CNmodeldf = TIC01, CNavg=VSAParams$CN[3], 
                declat=myflowgage$declat,declon=myflowgage$declon)
TIC03$qpeak=TIC03$Qpred/3600/24/1000*myflowgage$area/nTIclass*10^6 #m^3/sec
TIC03$sed=(TIC03$Qpred*TIC01$qpeak*myflowgage$area/nTIclass*100)^.56*MUSLE$KCPLSCFRG118[3]    # Eq. 4:1.1.1 SWAT Theory

TIC04$P=TIC03$Qpred+TIC04$P

TIC04 = CNmodel(CNmodeldf = TIC01, CNavg=VSAParams$CN[4], 
                declat=myflowgage$declat,declon=myflowgage$declon)
TIC04$qpeak=TIC04$Qpred/3600/24/1000*myflowgage$area/nTIclass*10^6 #m^3/sec
TIC04$sed=(TIC04$Qpred*TIC01$qpeak*myflowgage$area/nTIclass*100)^.56*MUSLE$KCPLSCFRG118[4]    # Eq. 4:1.1.1 SWAT Theory

TIC05$P=TIC04$Qpred+TIC05$P

TIC05 = CNmodel(CNmodeldf = TIC01, CNavg=VSAParams$CN[5], 
                declat=myflowgage$declat,declon=myflowgage$declon)
