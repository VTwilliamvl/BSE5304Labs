...
#WINNICUT RIVER AT GREENLAND, NR PORTSMOUTH, NH 01073785
myflowgage_id="01073785"
...
url="https://websoilsurvey.sc.egov.usda.gov/DSD/Download/AOI/dsjehl4eeoegx3hpmvzxbpph/wss_aoi_2023-02-15_20-28-59.zip"
...
mysoil=readOGR("wss_aoi_2023-02-15_20-28-59/spatial/soilmu_a_aoi.shp")
...
bmlt6 = 4.5   # referred to as SMFMX in SWAT input (Table 1), original = 4.5
...
legend("topright", c("Qmm(mm)", "Qpred(mm)"), col = c("black", "blue"),
       lty = 1:2, cex = 0.8)
TMWB$AWC=(0.45-0.15)*1000  
#Fld Cap = .45, Wilt Pt = .15, z=1000mm
...
plot(date,Qpred,col="black")
lines(date,Qmm,type = "l",col="red")
lines(date,Qpred,col="blue")
legend("topleft", c("P", "Qmm", "Qpred"), col = c("black", "red", "blue"),
       lty = 1:2, cex = 0.8)
detach(TMWB) # IMPORTANT TO DETACH
rm(list=c("Qpred","S"))
...
attach(TMWB)
plot(date,Qmm,type = "l",col="black")
lines(date,P,col="red")
lines(date,Qmm,type = "l",col="black")
lines(date,ET,type = "l",col="blue")
lines(date,Qpred,col="green")
lines(date,Excess, col="gray")
legend("topleft", c("P", "Qmm", "ET","Qpred","Excess"), col = c("red", "black", "blue", "green", "gray"),
       lty = 1:5, cex = 0.8)
NSE(Qmm, Qpred)
detach(TMWB) # IMPORTANT TO DETACH
rm(list=c("Qpred","S"))
...
attach(TMWB)
plot(date,AW,type = "l",col="purple")
detach(TMWB)
